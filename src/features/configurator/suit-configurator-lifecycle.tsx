...
export function SuitConfigurator({ configurationId }: { configurationId?: string }) {
  ...
  async function save() {
    ...
    try {
      ...
      const saved = await saveConfiguration({
        ...
        settings: draft.settings, createdBy: source?.createdBy ?? user.id, createdAt: draft.createdAt ?? now, updatedAt: now,
      });
  ...
  return <div className="space-y-7">
    {!canEdit && <div className="empty-state py-4" role="status"><p className="font-bold">Только просмотр</p><p className="muted mt-1 text-sm">Эта конфигурация принадлежит другому сотруднику. Изменять и удалять её может владелец или администратор.</p></div>}
    <PageHeading eyebrow="Сборка костюма" title="Конфигуратор" description="Параметры загружаются из групп и опций. 3D-просмотр исключён из этой версии по вашему указанию." actions={<Link href="/configurations" className="button button-secondary" onClick={(event) => { if (canEdit && dirty && !window.confirm("Изменения не сохранены. Выйти без сохранения?")) event.preventDefault(); }}>К конфигурациям</Link>} />

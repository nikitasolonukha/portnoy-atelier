...
import { SupabaseFabricAssetWriter } from "./supabase/supabase-fabric-asset-writer";
...
export async function getRepositories(): Promise<Repositories> {
  ...
  return {
    fabrics: new SupabaseFabricRepository(client),
    configurations: new SupabaseConfigurationRepository(client),
    imports: new SupabaseFabricImportLedger(client),
    assets: new SupabaseFabricAssetWriter(client),
    loadGroups: () => loadConfigurationGroups(client),
  };
}

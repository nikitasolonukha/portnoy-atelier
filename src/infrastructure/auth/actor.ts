...
export async function requireActor(): Promise<Actor> {
  ...
  if (env.APP_MODE === "demo") return { id: "00000000-0000-0000-0000-000000000001", fullName: "Admin Demo", role: "admin" };
  const client = await createClient();
  const { data: { user }, error } = await client.auth.getUser();
  if (error || !user) throw new ApiProblem("unauthorized", "Требуется авторизация", 401);
  const { data: profile } = await client.from("profiles").select("full_name,role,is_active").eq("id", user.id).maybeSingle();
  if (!profile?.is_active) throw new ApiProblem("account_inactive", "Учётная запись отключена", 403);
  return { id: user.id, fullName: profile.full_name.trim() || user.email || "Сотрудник", role: profile.role as Actor["role"] };
}
